...
# IoU thresholds and averaging the results from each run. It also supports the Pascal VOC metric (mAP @ 0.5) and custom user metrics.
...
parser.add_argument('--labels', help='Path to the label map file', default='labelmap.txt')
...
# Define arguments to show images and plots (if desired by the user)
show_img_arg = '' if show_imgs else ' -na'  # "-na" argument tells main.py NOT to show images
...
# Create folder to store output result files
...
# Create a dictionary to store overall mAP results and results for each class
mAP_results = {'overall': np.zeros(len(iou_threshes))}
for class_name in classes:
    mAP_results[class_name] = np.zeros(len(iou_threshes))  # Add each class to dictionary
...
for i, iou_thresh in enumerate(iou_threshes):
    ...
    # Save mAP results for this IoU value as a different folder name, then delete the modified script
    newpath = os.path.join(output_path, 'output_iou_%.2f' % iou_thresh)
    os.rename('output', newpath)
    os.remove('main_modified.py')

# mAP scores have been calculated for each IoU value. Now, average the mAPs and display them.
class_mAP_result = []
print('\n*** mAP Results ***\n')
print('Class\t\tAverage mAP @ 0.5:0.95')
print('---------------------------------------')
for class_name in classes:
    class_values = mAP_results[class_name]
    class_average = np.mean(class_values)
    class_mAP_result.append(class_average)
    print('%s\t\t%0.2f%%' % (class_name, class_average))
